...
def get_tag_values(filename):
    ...
    print 'Number of Ent sets is: ',mesh.getNumEntSets()
    tag_values = []  # list of tag values
    tally_groups = {}
    tally_objects=[]
    found_all_tags = 0
    for s in mesh_sets:
        #print s.iterate(iBase.Type.all, iMesh.Topology.triangle)  
        # get all the tags
                ...
                if 'tally' in group_name:
                    ...
                    for t in tags:        
                        if t.name == 'CATEGORY':
                            tag_h = mesh.getTagHandle(t.name)
                            tag = tag_h[s]
                            tally_object=tag_to_script(tag)
                            print tally_object
                            print 'Parents: ', s.getNumParents()
                            print 'Children: ', s.getNumChildren()        
                            print '-----------'
...
"""
function to check that tally group names exist and creates
a list of existing tallies
-------------------------------------------------
tag_values - list of tags from dagmc file
returns tally_data_list, a zipped pair of name and type of tallies
"""


def check_tallyname(tag_values):
    # loop over tags
    tally_list_type = []  # list of types of tallies
    tally_list_particle=[] #list of particles
    # loop over the tags in the file
    for tag in tag_values:
        if 'tally' in tag:
            try:
                ...
                raise Exception("tally group name is in wrong format!. '/' is missing in %s" %tag)  
            tally_list_type.append(tally[1])      
            try:
                tally_particle=tally[0].split(':')
            except:
                raise Exception("tally group name is in wrong format!. ':' is missing in %s" %tag)
            tally_list_particle.append(tally_particle[1])
            
    tally_list=zip(tally_list_particle, tally_list_type)
    return tally_list                         
